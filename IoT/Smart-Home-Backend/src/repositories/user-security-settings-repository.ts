...
/**
 * User security settings repository
 */
export class UserSecuritySettingsRepository {
  ...
  async findAll(userId: number): Promise<Record<string, any>> {
    const result = await this.pool.query(
      'SELECT setting_key, setting_value FROM user_security_settings WHERE user_id = $1',
      [userId]
    );
  ...
  async findOne(userId: number, key: string): Promise<any | null> {
    const result = await this.pool.query(
      'SELECT setting_value FROM user_security_settings WHERE user_id = $1 AND setting_key = $2',
      [userId, key]
    );
  ...
  async delete(userId: number, key: string): Promise<void> {
    await this.pool.query(
      'DELETE FROM user_security_settings WHERE user_id = $1 AND setting_key = $2',
      [userId, key]
    );
  }
}
