...
export class CharacterManager {
  ...
  async initPlayerCharacters(playerId: string): Promise<ICharacter[]> {
    const existing = await Character.find({ playerId }).lean() as ICharacter[];
  ...
  async getCharacters(playerId: string): Promise<ICharacter[]> {
    const chars = await Character.find({ playerId, isActive: true }).lean() as ICharacter[];
  ...
  async getTemplates(): Promise<ICharacter[]> {
    return Character.find({ playerId: null, isActive: true }).lean() as Promise<ICharacter[]>;
  }
}
